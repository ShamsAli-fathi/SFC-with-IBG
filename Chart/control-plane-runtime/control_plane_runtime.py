"""Plot mean controller admission runtime from the latest matching IBG traces."""
...
RUNS_DIR = ROOT / "runs"
DEFAULT_CSV = SCRIPT_DIR / "control_plane_runtime_IBG.csv"
DEFAULT_OUTPUT = SCRIPT_DIR / "control_plane_runtime.png"
DEFAULT_TITLE = "Control-Plane Decision Runtime on Small-Scale Topology: IBG-Exact"
IBG_ORANGE = "#ff7f0e"
...
def capture_latest(csv_path: Path, last_n: int, flows: int, stages: int, replicas: int) -> pd.DataFrame:
    ...
    for run_index, (path, iterations) in enumerate(
        ...
        for event in iterations:
            control_plane = event["summary"]["control_plane"]
            rows.append(
                {
                    "run": run_index,
                    "trace": path.name,
                    "timeslot": event["iteration"],
                    "admission_ms": control_plane["timing_ms"]["admission"],
                }
            )
...
def load_samples(path: Path) -> pd.DataFrame:
    ...
    required = {"run", "trace", "timeslot", "admission_ms"}
    missing = required.difference(frame.columns)
    if missing:
        raise ValueError(f"runtime CSV is missing columns: {', '.join(sorted(missing))}")
    if frame.empty:
        raise ValueError("runtime CSV has no samples")
    return frame


def aggregate(frame: pd.DataFrame) -> pd.Series:
    per_run = frame.groupby("run")["admission_ms"].mean()
    return pd.Series(
        {
            "mean": per_run.mean(),
            "std": per_run.std(ddof=1) if len(per_run) > 1 else 0.0,
        }
    )
...
def render(frame: pd.DataFrame, output: Path, title: str) -> Path:
    ...
    axis.text(0, mean, f"{mean:.1f} ms", ha="center", va="bottom")
    for index in (1, 2, 3):
        axis.text(index, mean * 0.04, "N/A", ha="center", va="bottom")
    axis.set_title(title)
    axis.set_ylabel("Mean admission runtime per slot (ms)")
    axis.set_xticks(
        [0, 1, 2, 3],

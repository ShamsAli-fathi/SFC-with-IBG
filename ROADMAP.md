...
Initial curation result: `EVIDENCE_SUMMARY.md` identifies the accepted calibration command and supported Kernel traces, separates modeled, localhost, live-cluster, and replay claims, and is opt-in-only. The Phase 2 synthetic calibration reproduces under current code. The historical Phase 3 trace still passes its live telemetry gate but predates the belief-retention change from 0.6 to 0.8. Three fresh supported-size retention-0.8 Kernel runs (seeds 2050--2052) replay exactly across 31 slots and 279 placements/observations, restoring current-HEAD mathematical parity. Their live server-overshoot results vary: seed 2050 records 89.90%, while seeds 2051 and 2052 each record 100%; the 96.42% pooled rate is descriptive and does not replace the existing per-trace 95% scheduling gate. One requested exploratory 10-flow/5-replica Kernel run reached equilibrium and replayed exactly across 10 slots and 300 hops, with 95.33% timing-tolerance coverage; its unsupported dimensions and incomplete/variable state-group summaries prevent it from extending the formal three-flow gate. No solver or datapath change is implied.

...
Status: in progress. The completed control-plane/learning-signal and nonnegative-observation-jitter branches below are part of Phase 4; reporting curation remains active and does not authorize Phase 5 or later work.
...
Status: revised locally; fresh normal-build live validation pending. Overall Phase 4 reporting curation remains active.

The active `half-normal-additive-v1` model replaces symmetric selected-processing jitter with $J_\theta=|Z_\theta|$, $Z_\theta\sim\mathcal N(0,\sigma_\theta^2)$, so a state/load baseline is never reduced by jitter. The user-fixed state-1-through-state-4 scales are 6, 5.25, 4, and 3.25 ms; do not revise them again without first reporting the evidence and proposed values. This branch is limited to the latency-observation model and its calibration; it does not change the exact decoupled solver, placement, selected-only boundary, posterior aggregation/retention 0.8, utility form, no-rejection policy, pair-cost deduction, 110 ms SLA threshold, strict `<0.033` equilibrium rule, or datapath implementation.

- The selected-latency sampler, matching per-state likelihood density, and expected-latency calculation use the same half-normal law. The likelihood assigns zero support below a state's deterministic state/load baseline.
- Deterministic sampler/PDF/likelihood fixtures, probability normalization/support checks, state/congestion ordering, utility crossings, sensitivity analysis, and selected-only/correlation tests pass. The full suite passes with 160 tests.
- The revised official 5,000-sample seed-2050 calibration passes: minimum classification is 97.88%, crossings remain 3/5/7/11, all sensitivity cases retain their target bands, and the modeled 110 ms SLA gate passes.
- The full local suite passes with 160 tests. Fresh normal-build live and replay evidence is still required for the revised scales. Trace `runs/ibg-experiment-20260719T125810Z.jsonl` remains accepted only for the superseded 8/7/5.5/4.5-ms profile and must not be claimed as current-head evidence.
- Current concern, pending user/professor direction: the two latest exploratory current-scale traces show only two false estimates across 126 selected observations for Stage-1 replicas 5 and 6, so the smaller scales may still make belief estimates too accurate. Larger physical jitter previously raised end-to-end latency and SLA violations; the prior 8/7/5.5/4.5-ms physical profile was also still judged too accurate, so its numbers cannot simply be reused as observation-only scales. A separate observation-only signal-jitter model is a possible resolution, but is not authorized or implemented; it must retain nonnegative noise, clearly separate SLA/utility inputs from learning-signal inputs, use a likelihood matching the resulting signal distribution, and calibrate to a declared prediction-error target.
...
Gate: pending fresh live evidence and a decision on the jitter/SLA modelling boundary. The distribution and revised parameters are recorded; sampling, likelihood, calibration, and local tests agree, while the normal-build supported Kernel gate and exact replay remain to be run. Solver, placement, learning boundary, utility, SLA, pair-cost, and equilibrium invariants remain unchanged.

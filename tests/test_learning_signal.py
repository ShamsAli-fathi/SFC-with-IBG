...
def observation(stage, flow_id, replica_id, *, measured=12.5, state=4):
    return Observation(
        stage=stage,
        flow_id=flow_id,
...
def test_diagnostic_telemetry_does_not_change_learning_signal_footprint():
    baseline = [observation(1, 1, 2, measured=12.5, state=4)]
    changed_diagnostics = [observation(1, 1, 2, measured=999.5, state=1)]

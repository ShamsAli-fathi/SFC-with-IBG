...
Expansion Phase 3 remains implemented locally, and Phase 4 evidence curation is in progress. The existing Kubernetes HTTP path is the explicit and independently runnable `kernel` baseline. The historical supported trace still passes the live supported-size statistical checks. The fresh retention-0.8 trace restores exact current-HEAD replay parity across 11 slots, but its 89.90% server-overshoot pass rate is below the 95% live scheduling threshold, so it does not replace the accepted live Kernel gate. The active linear utility, selected-only observation, hard 250 ms SLA, decoupled exact solver, and no-rejection behavior are unchanged. The deferred 50-load calibration remains required before accepted per-replica utility values above 12 are used. DPDK/VPP is still unimplemented and unclaimed.

## Next action

Continue Phase 4 from the opt-in `EVIDENCE_SUMMARY.md`. The next evidence decision is user-directed: either retain the accepted historical live Kernel statistical gate alongside the fresh current-HEAD replay result, or collect more current retention-0.8 Kernel runs to assess the 95% scheduling threshold. Keep every result tied to its source command/trace, seed, dimensions, mode, code-version boundary, and validation category. Do not read or edit `Tutorial.md`, `Report.md`, or `EVIDENCE_SUMMARY.md` in a future task unless explicitly asked, and do not rewrite generated artifacts or change IBG/datapath behavior. The subsequent Phase 5 remains the design-only DPDK/VPP boundary and host preflight.

## New-thread handoff prompt

> We are continuing after completion of Phase 3 in `/home/vhakami/Desktop/projects/vesal/SFC-with-IBG` on branch `IBG`. Read `AGENTS.md`, `ARCHITECTURE.md`, `DECISIONS.md`, `ROADMAP.md`, and `STATUS.md`. Do not read or edit `Tutorial.md`, `Report.md`, or `EVIDENCE_SUMMARY.md` unless explicitly requested. Phases 1--3 are implemented and Phase 4 evidence curation is in progress. The HTTP/Kubernetes path is explicit `kernel`. The historical supported seed-2050 trace passes the live state/congestion/classification/timing gate but predates belief retention 0.8. The fresh seed-2050 trace `runs/ibg-experiment-20260715T122305Z.jsonl` reaches equilibrium in 11 slots and replays with zero placement, observation, belief, and mathematical drift under current HEAD; its live server-overshoot rate is 89.90%, below the 95% threshold. Keep the historical accepted live gate and fresh current-head replay evidence distinct. The hard 250 ms SLA, linear utility, selected processing-latency signal, decoupled exact solver, and no-rejection behavior remain unchanged. A deferred 50-load utility calibration remains required before accepted per-replica loads above 12 are used. Keep evidence claims tied to identified commands/traces, seeds, dimensions, mode, code version, and validation category; do not rewrite generated artifacts or change behavior. DPDK/VPP design and host preflight are Phase 5; do not add privileged DPDK/VPP resources, pair-dependent optimization, rejection, or coupled behavior without explicit scope and a passing preflight.

...
Begin Phase 4 with a design-only DPDK/VPP integration boundary and host preflight. Document the exact forwarding topology, approved DPDK I/O backend, Linux-facing interfaces, resource ownership, IOMMU/VFIO, hugepages, CPU/NUMA, Kubernetes device handling, lifecycle, readiness, cleanup, failure behavior, and rollback before adding any privileged resources. Keep `kernel` independently runnable and do not change the solver, selected-only observation contract, SLA, rejection, or coupled behavior.

## New-thread handoff prompt

> We are continuing after completion of Phase 3 in `/home/vhakami/Desktop/projects/vesal/SFC-with-IBG` on branch `IBG`. Read `AGENTS.md`, `ARCHITECTURE.md`, `DECISIONS.md`, `ROADMAP.md`, and `STATUS.md`. Do not read or edit `Tutorial.md` unless explicitly requested; `Report.md` is opt-in only. Phases 1--3 are implemented. The existing HTTP/Kubernetes path is explicit `kernel`; its supported nine-slot seed-2050 run passed live state/congestion/classification/timing checks and replayed with zero mathematical drift through `scripts/phase3_kernel_baseline.py`. The hard 250 ms SLA, linear utility, selected processing-latency signal, decoupled exact solver, and no-rejection behavior remain unchanged. A deferred 50-load utility calibration remains required before accepted per-replica loads above 12 are used. The next work is Phase 4 design and host preflight only: specify DPDK/VPP resource ownership, topology, interfaces, lifecycle, readiness, failure, cleanup, and rollback before implementation. Do not add privileged DPDK/VPP resources, pair-dependent optimization, rejection, or coupled behavior without explicit scope and a passing preflight.

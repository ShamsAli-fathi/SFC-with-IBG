...
Expansion Phase 3 is complete locally. The existing Kubernetes HTTP path is the explicit and independently runnable `kernel` baseline, complete mode/request-overhead/provenance telemetry is recorded, the live supported-size statistical gate passes, and captured signals replay with zero mathematical drift. The active linear utility, selected-only observation, hard 250 ms SLA, decoupled exact solver, and no-rejection behavior are unchanged. The deferred 50-load calibration remains required before accepted per-replica utility values above 12 are used. DPDK/VPP is still unimplemented and unclaimed. A new user-directed evidence and report-curation Phase 4 now precedes all DPDK/VPP work.

## Next action

Begin Phase 4 by helping the user curate and explain existing simulation and `kernel` evidence. Keep every result tied to its source command/trace, seed, dimensions, mode, and validation category; clearly distinguish replay parity, synthetic calibration, localhost FastAPI checks, and live Kubernetes telemetry. This phase is user-directed: do not read or edit `Tutorial.md` or `Report.md` unless explicitly asked, and do not rewrite generated artifacts or change IBG/datapath behavior. The subsequent Phase 5 remains the design-only DPDK/VPP boundary and host preflight.

## New-thread handoff prompt

> We are continuing after completion of Phase 3 in `/home/vhakami/Desktop/projects/vesal/SFC-with-IBG` on branch `IBG`. Read `AGENTS.md`, `ARCHITECTURE.md`, `DECISIONS.md`, `ROADMAP.md`, and `STATUS.md`. Do not read or edit `Tutorial.md` unless explicitly requested; `Report.md` is opt-in only. Phases 1--3 are implemented. The existing HTTP/Kubernetes path is explicit `kernel`; its supported nine-slot seed-2050 run passed live state/congestion/classification/timing checks and replayed with zero mathematical drift through `scripts/phase3_kernel_baseline.py`. The hard 250 ms SLA, linear utility, selected processing-latency signal, decoupled exact solver, and no-rejection behavior remain unchanged. A deferred 50-load utility calibration remains required before accepted per-replica loads above 12 are used. Phase 4 is user-directed evidence and report curation: keep claims tied to identified commands/traces, seeds, dimensions, mode, and validation category; do not rewrite generated artifacts or change behavior. Only read/edit `Tutorial.md` or `Report.md` when the user explicitly asks. DPDK/VPP design and host preflight are now Phase 5; do not add privileged DPDK/VPP resources, pair-dependent optimization, rejection, or coupled behavior without explicit scope and a passing preflight.

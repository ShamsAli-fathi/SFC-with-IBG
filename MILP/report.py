...
def SLA_v(embed_dict, replica_list):
    ...
    for index, embeds in embed_dict.items():
        count_of_ones = sum(
            ...
            if replica_list[(index + 1, rep)].state == 1
        )
        if count_of_ones > 0:
            violation_count += 1
...
def csv_gen_SLA(violation_count, hash_value, filename='sla_violations_milp.csv'):
    """
...
def csv_gen_util(violation_count, hash_value, filename='aggregate_utility_milp.csv'):
    """
    Appends violation_count to a CSV file under a column named after hash_value.
...
def csv_gen_jain(violation_count, hash_value, filename='jain_index_milp.csv'):
    """
    Appends violation_count to a CSV file under a column named after hash_value.
...
def csv_gen_time(violation_count, hash_value, filename='time_milp.csv'):
    """
    Appends violation_count to a CSV file under a column named after hash_value.

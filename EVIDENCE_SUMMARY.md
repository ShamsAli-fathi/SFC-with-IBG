...
| Retention-0.8 Kernel evidence set | Seed 2050: `./scripts/run_experiment.py --seed 2050 --flow 3 --stage 3 --replica 5 --max-iterations 100`, `runs/ibg-experiment-20260715T122305Z.jsonl`; seeds 2051/2052: same command with `--skip-build`, traces `runs/ibg-experiment-20260715T123505Z.jsonl` and `runs/ibg-experiment-20260715T123533Z.jsonl`; each validated 2026-07-15 with `.venv/bin/python scripts/phase3_kernel_baseline.py <trace>` | Under the then-active strict-0.04 equilibrium threshold, all three retention-0.8 runs reached equilibrium and replayed exactly: 31 slots, 279 placements/observations, and zero belief/mathematical drift. Seeds 2051 and 2052 each passed every live check, including 100% server-overshoot tolerance across 81 and 99 hops; seed 2050 replayed exactly but passed 89/99 hops (89.90%). | The active threshold is now strict 0.03, so these traces do not establish current-HEAD stopping/replay evidence until a fresh run is captured. The pooled live rate is 269/279 (96.42%), but that is descriptive evidence only: it does not redefine the existing per-trace 95% scheduling gate or erase the seed-2050 failure. This remains lightweight HTTP/Kubernetes Kernel evidence, not a DPDK/VPP or hardware claim. |
...
| Exploratory 12-flow CSV run and open metric concern | `./scripts/run_experiment.py --flow 12 --stage 3 --replica 5 --max-iterations 100 --skip-build --csv 1`; seed 2050; trace `runs/ibg-experiment-20260715T133726Z.jsonl`; host-side CSVs under ignored `figures/`; validated 2026-07-15 | Under the then-active strict-0.04 equilibrium threshold, reached equilibrium in 8 slots with 288 selected hops and exact replay. From slot 1 to 8, selected-state quality improved (mean 2.67 to 3.39; state-1 selections 7 to 0; state-4 selections 11 to 19). Predicted processing utility before transport rose 2285 to 2639 and realized processing utility before transport rose 2163 to 2759. | The exported `aggregate_utility.csv` records `aggregate_utility_total`, which subtracts summed live request-minus-processing overhead. That penalty rose 654 to 1077, so the reported aggregate fell 1631 to 1562 despite improved selections. Do not use it alone as learning evidence. The additive `realized_end_to_end_utility.csv` now exports the distinct actual processing-minus-live-transport outcome without redefining legacy CSVs; it is not guaranteed to increase every run. |
...
4. The fresh supported-size retention-0.8 evidence set closed that replay gap under the then-active strict-0.04 stopping condition: all 31 slots across seeds 2050, 2051, and 2052 replay exactly.
...
The active equilibrium threshold was user-changed from strict 0.04 to strict 0.03 on 2026-07-15. This changes only the run stopping condition. A new trace is required before claiming current-HEAD equilibrium/replay evidence under that threshold.

Use wording such as: “The accepted Phase 3 Kernel trace passes its live statistical telemetry gate. Three retention-0.8 traces replayed with zero mathematical drift under the prior 0.04 stopping condition; two meet the per-trace 95% live scheduling threshold, while one records 89.90%, showing live scheduling variability rather than a mathematical discrepancy.”

## Open aggregate-utility reporting concern

The current `aggregate_utility_total` first sums belief-predicted stage utility for selected placements, then subtracts the per-flow sum of every selected hop's live request-minus-processing overhead. It is therefore a hybrid end-to-end metric, rather than a processing-only learning metric. The 12-flow CSV run shows that its transport component can mask a clear improvement in selected true states and processing utility. The user has flagged this as unresolved. `realized_end_to_end_utility.csv` is now the additive, transport-inclusive actual-outcome series; it exports the existing realized processing utility minus the same live transport penalty. It must not be claimed to increase monotonically. Do not silently redefine the legacy CSV or alter the solver, belief update, latency signal, SLA, or transport deduction without explicit authorization.

## Claim boundaries

The evidence supports a small, controlled decoupled IBG testbed with exact continuation play at the validated 3-flow/3-stage/5-replica size, selected-only processing-latency learning signals, linear latency utility, a hard 175 ms end-to-end SLA, and a strict 0.03 per-belief-entry equilibrium stopping threshold. The existing traces retain their recorded older SLA/stopping-rule code-version boundaries.

...
The independent pair-residual/SLA symptom remains open. In the repaired 19-slot 15x8 trace, final physical/pair/end-to-end means were 71.04/62.06/133.10 ms per flow and 14/15 flows violated SLA; all-slot pair-cost mean/p95 were 59.05/111.01 ms. Thus the memory repair removes the deterministic controller failure and reduces admission time, but it does not establish a cause or remedy for selected public-forwarder/Kubernetes route variance. It remains separate from observation jitter and must not be addressed by changing the SLA, utility, jitter scales, solver, or pair-cost definition.
